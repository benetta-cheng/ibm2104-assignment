...
if (isset($_GET['id']) && $_GET['id'] !== "") {
        ...
        if (!empty($resultScheck) && $resultScheck->num_rows > 0) {
            ...
            if (!empty($result) && $result->num_rows > 0) {
                while ($staffSchedules = mysqli_fetch_assoc($result)){
                    if(strtotime($staffSchedules['depart_dateTime']) <= $arriveTime && strtotime($staffSchedules['arrive_dateTime']) >= $departTime){
                        $allowSave['pilot'] = 0;
    ...
    if (isset($_GET['flightAttendantID']) && $_GET['flightAttendantID'] !== ""){
        ...
        foreach($attendants as $attendant){
            ...
            if (!empty($resultScheck) && $resultScheck->num_rows > 0) {
                $query = "SELECT flight_attendant_team.*, flight_schedules.depart_dateTime, flight_schedules.arrive_dateTime FROM flight_attendant_team INNER JOIN flight_teams ON flight_attendant_team.flight_team = flight_teams.id INNER JOIN flight_schedules ON flight_teams.schedule_no = flight_schedules.id WHERE flight_attendant_team.flight_attendant = $attendant";
                $result = $connection->query($query);
                if (!empty($result) && $result->num_rows > 0) {
                    while ($staffSchedules = mysqli_fetch_assoc($result)){
                        if(strtotime($staffSchedules['depart_dateTime']) <= $arriveTime && strtotime($staffSchedules['arrive_dateTime']) >= $departTime){
        ...
        if($update==1){
            ...
            if (!empty($result) && $result->num_rows > 0) {
                ...
                foreach($allowSaveQuery['attendant'] as $saveQuery){
                    $updateAttendant = "UPDATE flight_attendant_team SET flight_attendant=$saveQuery WHERE flight_team = $teamID";
...
<form action = "flightTeam_search.php" method = "GET">
    <div class="form-row">
        ...
        <input type="text" class="form-control" name="pilotID" placeholder="Enter the staff ID for pilot" value= "<?php echo $displayPilots['pilot'] ?? "";?>" required>
        ...
        <input type="text" class="form-control" name="coPilotID" placeholder="Enter the staff ID for co-pilot" value="<?php echo $displayPilots['copilot'] ?? "";?>" required>
